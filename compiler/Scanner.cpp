...
// ensure binary read on windows
	...
	if (bufLen == fileLen && CanSeek()) Close();  // if input stack empty
}
...
Scanner::Scanner(FILE* s, bool b) {
  buffer = new Buffer(s,b);
	Init();
}
...
void Scanner::Init() {
	...
	for (i = 49; i <= 57; ++i) start.set(i, 12);
	start.set(48, 13);
	start.set(46, 6);
	start.set(34, 8);
	start.set(35, 10);
	...
	keywords.set(L"extern", 5);
	keywords.set(L"int", 12);
	keywords.set(L"float", 13);
	keywords.set(L"void", 14);
	keywords.set(L"if", 18);
	keywords.set(L"else", 19);
	keywords.set(L"for", 20);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 10:
			case_10:
			if (ch == 10 || ch == 13) {AddCh(); goto case_11;}
			else if (ch <= 9 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= 65535)) {AddCh(); goto case_10;}
			else {goto case_0;}
		case 11:
			case_11:
			{t->kind = 39; break;}
		...
		case 13:
			recEnd = pos; recKind = 2;
			if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_12;}
			else if (ch == L'x') {AddCh(); goto case_2;}
			else if (ch == L'.') {AddCh(); goto case_14;}
			else {t->kind = 2; break;}
		case 14:
			case_14:
